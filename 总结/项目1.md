...
仓库里有 new-requirement 这类 skill 专门路由「新代码该落在哪、规模多大」，防止 Agent 越界乱改；
...
- “依赖提升”、“统一安装”
- 各个应用没有自己的 node_modules ，所有依赖被提升到根目录统一管理
- 一次 npm install，所有依赖统一安装、统一管理，避免重复下载和版本碎片化

真正需要 monorepo 的场景：

- 共享代码包 — 比如前后端共用一个 packages/shared/ 放类型定义、工具函数
- 共享依赖版本管理 — 比如两边都用了 zod，需要保证版本一致
- 跨包引用 — 比如 web 里 import { ApiTypes } from "@chatbot/shared"
- 多包发布 — 比如同时维护多个 npm 包，需要统一版本号
