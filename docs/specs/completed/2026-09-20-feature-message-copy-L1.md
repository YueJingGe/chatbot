...
|复制反馈|点击后菜单关闭，无全局 toast（先保持最小改动）|L1 范围控制；如后续需要反馈可再增强|
...
- DOM 环境：jsdom。
- 测试文件：与源码同目录下的 `__tests__/*.test.{ts,tsx}`。
- 新增文件：
...
- [x] 渲染 assistant 消息时，hover 气泡后可见复制按钮与下拉箭头；点击箭头展开菜单。
- [x] assistant 菜单中点击「复制」时，调用 `copyToClipboard(stripMarkdown(message.content))`。
- [x] assistant 菜单中点击「复制为 Markdown」时，调用 `copyToClipboard(message.content)`。
- [x] 点击页面其他区域，展开的菜单关闭。
...
- `web/src/components/MessageList.tsx`：为每条消息增加复制入口与交互逻辑。
- `web/src/components/MessageList.module.less`：复制 icon、下拉菜单的样式与定位。
- `web/src/components/__tests__/MessageList.test.tsx`：组件测试。
