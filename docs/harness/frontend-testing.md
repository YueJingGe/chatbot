...
> 新增、修改或修复前端行为时必读。工具链与 React 组件测试模式沿用经外部参考项目验证的 Vitest + Testing Library 组合，并按本项目规模裁剪。
...
新增、修改或修复工具函数、hook、组件交互或数据行为时，按 Red-Green-Refactor 执行：

1. 先写一个只表达单项预期的测试，或一个能复现 bug 的测试。
2. 运行测试，确认它因目标行为缺失或 bug 存在而失败（Red）。保留失败输出作为证据。
3. 编写满足该测试的最小实现，或修复最小代码使测试通过（Green）。
4. 在保持全绿前提下重构；继续下一个行为或补边界测试。

纯布局、颜色或响应式样式变更不强制单元测试，仍按 `frontend-visual-verification` 完成视觉验证。

### Bug Fix 的 TDD 要求

- 必须先有**复现 bug 的失败测试**，再修改实现代码。
- 必须保留测试失败的命令输出/截图作为证据。
- 修复后同一测试必须通过。
- 没有失败证据，不算 TDD 修 bug。
...
- [ ] 新增、修改或修复的行为具有同目录测试，且已观察到预期的 Red → Green；修 bug 时保留复现测试的失败输出。
