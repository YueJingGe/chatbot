...
description: 新需求启动 + 显式路由。仅在用户明确说「新需求」「新功能」「加个 xxx」或显式调用 /new-requirement 时触发；不要从"帮我做 xxx""做 xxx"等模糊表述自动推断为新需求。先按规模自动判断走 L0 小（≤1 文件 ≤10 行）→ 直接改；L1 中（≤3 文件 ≤50 行）→ 简化 spec；L2 大 → 完整 spec + exec-plan。给每个级别说判断依据，用户可推翻。改/修 bug/查问题/普通工具函数等不属于"新需求"的动作不要触发此 skill。
---
...
读 `AGENTS.md` 确认规范已加载。本 skill **仅在用户明确表达新需求意图时触发**，触发信号包括：

- 明确说"新需求""新功能""加一个 xxx""新增 xxx"。
- 显式调用：`/new-requirement` 或"走 new-requirement"。

**不要触发**的情况（直接按普通任务处理，可仍调用 `test-driven-development` 写测试）：

- "帮我做 xxx""做 xxx""实现 xxx""改 xxx"等模糊表述。
- 纯工具函数、样式调整、bug 修复、代码查询、重构。
